...
class TestAudioExtractor:
  ...
  def test_is_audio_safe_reflects_processing_state_without_doing_work(self, tmp_path, monkeypatch):
    ...
    assert is_audio_safe(segment) is False, "not processed yet"
    assert len(ffmpeg_calls) == 0, "is_audio_safe must never run ffmpeg itself"

    assert process_segment_audio(segment) is True
    assert is_audio_safe(segment) is True
